...
func TestTeleDB(t *testing.T) {
	...
	cases := []struct {
		...
		{"error", `tele { listen "tcp://" {} }`, func(env *tenv) {
			...
			var tm vender_api.Telemetry
			require.NoError(t, proto.Unmarshal(b, &tm))
			require.NoError(t, onTelemetry(env.ctx, env.db, tm.VmId, &tm))

			var count int
			_, err = env.db.QueryOne(pg.Scan(&count), `select count(*) from trans`)
			require.NoError(t, err)
			assert.Equal(t, 0, count)

			var errorRow MError
			_, err = env.db.QueryOne(&errorRow, `select * from error where vmid=?`, tm.VmId)
			require.NoError(t, err)
			...
			var ingest MIngest
			_, err = env.db.QueryOne(&ingest, `select * from ingest where vmid=?`, tm.VmId)
			require.NoError(t, err)
		...
		{"state", `tele { exec_on_state="false" listen "tcp://" {} }`, func(env *tenv) {
			...
			_, err := env.db.QueryOne(pg.Scan(&count), `select count(*) from state`)
			require.NoError(t, err)
			assert.Equal(t, int(0), count)

			require.NoError(t, onState(env.ctx, env.db, vmid, vender_api.State_Boot))
			_, err = env.db.QueryOne(st, `select * from state where vmid=?`, vmid)
			require.NoError(t, err)
			assert.Equal(t, vmid, st.VmId)
			assert.Equal(t, vender_api.State_Boot, st.State)

			require.NoError(t, onState(env.ctx, env.db, vmid, vender_api.State_Nominal))
			_, err = env.db.QueryOne(st, `select * from state where vmid=?`, vmid)
			require.NoError(t, err)
			assert.Equal(t, vmid, st.VmId)
			assert.Equal(t, vender_api.State_Nominal, st.State)
		}},
		{"telemetry", `tele { listen "tcp://" {} }`, func(env *tenv) {
			...
			var tr MTrans
			_, err = env.db.QueryOne(&tr, `select * from trans where vmid=?`, tm.VmId)
			require.NoError(t, err)
		...
		{"func-vmstate-string", `tele { listen "tcp://" {} }`, func(env *tenv) {
			t := env.t
			for i, expect := range vender_api.State_name {
				var dbtext string
				_, err := env.db.QueryOne(&dbtext, `select vmstate(?)`, i)
				if assert.NoError(t, err) {
					assert.Equal(t, expect, dbtext)
				}
			}
		}},
	}
